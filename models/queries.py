...
def get_kpis():
    return """
        with c_anon_users as (
            select id id_date, userlist->'rtc_online_users'->'anon_users' users
            from rtc_online_users 
            where id between '2022-10-01'::date and '2022-10-31'::date
                and json_array_length(userlist->'rtc_online_users'->'anon_users') > 0
        ),
        c_reg_users as (
